...
raw_example <- bind_rows(raw_y1, raw_y2) %>% 
  rename(area = state) %>% 
  select(area, common_name, method, waterbody_type, year, effort, total_length, weight)
```
...
calculate_cpue <- function(df){
  stopifnot(c("area", "common_name", "method", "waterbody_type", 
              "year", "effort") %in% colnames(df))
  cpue <- df %>% 
    mutate(effort = ifelse(method %in% c("boat_electrofishing", 
                                       "tow_barge_electrofishing", 
                                       "raft_electrofishing", "trawl"), 
                           effort / 3600, 
                           effort)) %>%
    add_count(area, common_name, method, waterbody_type, year) %>% 
    mutate(cpue = n / effort) %>% 
    group_by(area, common_name, method, waterbody_type) %>% 
...
calculate_lf <- function(df){
  ...
  lf <- raw_example %>% 
    ...
    group_by(area, common_name, method, waterbody_type, year) %>% 
    count(gcat) %>% 
    mutate(lenfreq = n / sum(n)) %>% 
    group_by(area, common_name, method, waterbody_type, gcat) %>% 
...
calculate_rw <- function(df){
  stopifnot(c("area", "common_name", "method", "waterbody_type", "year", 
