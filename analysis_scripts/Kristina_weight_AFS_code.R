...
weight <- WSlit
length <- PSDlit
# the weight table has been updated since i ran this and names have changed, they can now match up with length
# because the length equations for Rainbow Trout and Rainbow Trout lentic/lotic are all the same but now they can
#be run through both length and weight with those names, also have to do this for Walleye, Muske and paddelfish (overall)

data$common_name[data$common_name=="Spotted Bass"] <- "Spotted Bass (overall)"
...
names(old_summary_single_na)[names(old_summary_single_na) == "X5."] <- "5%"
names(old_summary_single_na)[names(old_summary_single_na) == "X25."] <- "25%"
names(old_summary_single_na)[names(old_summary_single_na) == "X50."] <- "50%"
names(old_summary_single_na)[names(old_summary_single_na) == "X75."] <- "75%"
names(old_summary_single_na)[names(old_summary_single_na) == "X95."] <- "95%"
...
new_summary_df <- NorthAmerica.results.weight %>% 
  mutate(mean = round(mean, 1), 
         ...
         common_name = case_when(
           ...
           TRUE ~ common_name)) %>%    # mutate closes here
  filter(n > 4)
